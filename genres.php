...
if($_GET['page']) {
$page = $_GET['page'];
$url = 'https://'.$spatch.'/genres/'.$q.'/page/'.$page.'/';
} else {
$url = 'https://'.$spatch.'/genres/'.$q.'/';
}

$urlx = strstr(grab($url), '<body');
$title = ''.$sitename.' - Drama Korea Genre '.$q.' Subtitle Indonesia';
$description = ''.$sitename.' - Drama Genre '.$q.' '.strip_tags($deskrip).'';
$keyword = 'Drama Genre '.$q.', '.$key;
...
$paging = str_ireplace('Sebelumnya', '', $paging);
$paging = str_ireplace('Berikutnya ', '', $paging);
