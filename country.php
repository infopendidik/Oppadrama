...
if($_GET['page']) {
$page = $_GET['page'];
$url = 'https://'.$spatch.'/series/?page='.$page.'&country='.$q.'&order=update/';
} else {
$url = 'https://'.$spatch.'/series/?country='.$q.'&order=update/';
}

$urlx = strstr(grab($url), '<body');
$title = ''.$sitename.' - Drama '.$q.' Subtitle Indonesia';
$description = ''.$sitename.' - '.$q.' '.strip_tags($deskrip).'';
$keyword = 'Drama '.$q.', '.$key;
...
$paging = str_ireplace('?page=', '/country/'.$q.'/', $paging);
$paging = str_ireplace('&country='.$q.'&order=update', '', $paging);
